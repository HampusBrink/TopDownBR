...
public class PlayerMovement : NetworkBehaviour
{
    ...
    private void AssignComponents()
    {
        ...
        if (IsOwner) _rb.isKinematic = false;
    ...
    private void Update()
    {
        if (!GameManager.Instance.GameStarted && !IsOffline) return;
    ...
    {
        Vector3
            cross = Vector3.Cross(move,
                Vector3.up); // Cross product magic to get the vector we want to move on the normal.
    ...
    private void ApplyMovement()
    {
        ...
        _velocity = input * _multipliedSpeed;
